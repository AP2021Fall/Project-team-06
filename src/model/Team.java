...
public class Team {
    ...
    private ArrayList<Notification> chat = new ArrayList<>();
    ...
    public String showScoredoard() {
        ...
        for(User user : members){
            scoreboard.put(user.getName(), user.getScore());
    ...
    public String showBoard(String boardName) {
        ...
        String output = "Board name: "+board.getName();
        output += "\nBoard copletion : "+board.getProgress();
        output += "\nBoard failed: "+board.getFailedProgress();
        output += "\nBoard leader: "+leader.getName();
        output += "\nBoard tasks: "+board.showTasks();
        return output;
    }

    public String showTask(int id) {
        Task task = Task.getTaskById(name, id);
        String output = task.getTitle()+": id"+task.getId()+",creation date : ";
        output += task.getCreationDate()+",deadline :"+task.getDeadline()+",assign to :";
        ArrayList<User> assignUser = task.getAssignedUsers();
        for(User user : assignUser){
